...
fn test_onchain_to_onchain_claim() {
	...
	// Then, broadcast these to C, which should update the monitor downstream on the B<->C
	// channel.
	// Finally, check that C will claim the HTLC output if B's latest commitment transaction
	// gets broadcast.
	...
	// Create some initial channels
	let _chan_1 = create_announced_chan_between_nodes(&nodes, 0, 1, InitFeatures::known(), InitFeatures::known());
	let chan_2 = create_announced_chan_between_nodes(&nodes, 1, 2, InitFeatures::known(), InitFeatures::known());
	let chan_3 = create_announced_chan_between_nodes(&nodes, 2, 3, InitFeatures::known(), InitFeatures::known());
	...
	let d_txn = nodes[3].tx_broadcaster.txn_broadcasted.lock().unwrap().clone(); // ChannelManager : 2 (commitment tx, HTLC-Success tx), ChannelMonitor : 1 (HTLC-Success tx)
	...
	{
		let mut c_txn = nodes[2].tx_broadcaster.txn_broadcasted.lock().unwrap();
		// ChannelMonitor: claim tx
		assert_eq!(c_txn.len(), 1);
		check_spends!(c_txn[0], chan_3.3); // C local commitment tx, issued by ChannelManager
		c_txn.clear();
	}
	...
	// Broadcast B's commitment tx on C's chain to see if we are able to claim inbound HTLC with our HTLC-Success tx
	let commitment_tx = get_local_commitment_txn!(nodes[1], chan_2.2);
	mine_transaction(&nodes[2], &commitment_tx[0]);
	check_closed_event!(nodes[2], 1, ClosureReason::CommitmentTxConfirmed);
	let c_txn = nodes[2].tx_broadcaster.txn_broadcasted.lock().unwrap();
	...
	assert_eq!(c_txn[0].input[0].witness.clone().last().unwrap().len(), OFFERED_HTLC_SCRIPT_WEIGHT);
	assert!(c_txn[0].output[0].script_pubkey.is_v0_p2wpkh()); // direct payment
